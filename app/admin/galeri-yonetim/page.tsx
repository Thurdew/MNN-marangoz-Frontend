...
export default function GaleriYonetimPage() {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/galeri/${editModal.oge._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`http://localhost:5000/api/galeri/${deleteModal.ogeId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
