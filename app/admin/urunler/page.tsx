...
export default function UrunYonetimPage() {
  ...
  const fetchUrunler = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${BACKEND_URL}/api/urunler`, {
        headers: {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/urunler/${editModal.urun._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/urunler/${deleteModal.urunId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
